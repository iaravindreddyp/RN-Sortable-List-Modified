...
export default class SortableList extends Component {
  ...
  _setOrderOnMove() {
    ...
    if (
      ...
      const { data } = this.state
      let flag = 0
      let isReplacable = true
      if (activeRowIndex < rowUnderActiveIndex) {
        let key = order[rowUnderActiveIndex + 1]
          ? order[rowUnderActiveIndex + 1]
          : null
        if (key && data[key].isDisabled) {
          isReplacable = false
        }
      } else if (activeRowIndex >= rowUnderActiveIndex) {
        let key = order[rowUnderActiveIndex - 1]
          ? order[rowUnderActiveIndex - 1]
          : null
        if (key && data[key].isDisabled) {
      ...
      if (isNeighbours) {
        if (data && (!data[rowUnderActiveKey].isDisabled || isReplacable)) {
          flag = 1
        ...
        if (
          data &&
          data[rowUnderActiveKey] &&
          (!data[rowUnderActiveKey].isDisabled || isReplacable)
        ) {
          flag = 1
